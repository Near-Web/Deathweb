...
/obj/machinery/bot/mulebot/Topic(href, href_list)
	...
	if ((in_range(src, usr) && istype(src.loc, /turf)) || (istype(usr, /mob/living/silicon)))
		...
		switch(href_list["op"])
			...
			if("destination")
				refresh=0
				var/new_dest = sanitize_name(input("Enter new destination tag", "Mulebot [suffix ? "([suffix])" : ""]", destination) as text|null, MAX_NAME_LEN, TRUE, FALSE)
				refresh=1
				if(new_dest)
			...
			if("setid")
				refresh=0
				var/new_id = sanitize_name(input("Enter new bot ID", "Mulebot [suffix ? "([suffix])" : ""]", suffix) as text|null, MAX_NAME_LEN, TRUE, FALSE)
				refresh=1
				if(new_id)
					suffix = new_id
					name = "Mulebot ([suffix])"
			...
			if("sethome")
				refresh=0
				var/new_home = sanitize_name(input("Enter new home tag", "Mulebot [suffix ? "([suffix])" : ""]", home_destination) as text|null, MAX_NAME_LEN, TRUE, FALSE)
				refresh=1
				if(new_home)
