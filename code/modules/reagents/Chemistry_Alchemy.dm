...
/datum/reagent/alchemy/blessing_baccus/on_mob_life(var/mob/living/M as mob, var/alien)
	...
	if(!H.has_penis())
		return
	H.penis_size += alchemy_skill + volume
	if(H.penis_size > 30)
		H.mutilate_genitals()
...
/datum/reagent/alchemy/bridge_of_ttf/on_mob_life(var/mob/living/M as mob, var/alien)
	if(!istype(M))
		return
	if(!istype(M, /mob/living))
		return
...
/datum/reagent/alchemy/bridge_of_ttf/on_remove(var/mob/living/carbon/human/H as mob)
	H.bridged = FALSE
	return
