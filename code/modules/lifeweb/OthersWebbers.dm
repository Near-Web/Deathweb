...
/obj/structure/stool/bed/chair/cross/buckle_mob(mob/living/carbon/human/M as mob, mob/living/carbon/human/user as mob)
	if(M.crucified)
		if(ishuman(user) && user.wear_id)
			var/obj/item/card/id/idcard = user.wear_id
			if(istype(idcard, /obj/item/card/id/chaplain))
				playsound(src.loc, 'sound/LW2/lw_free.ogg', 100, 0, -1)
				M.buckled = null
				buckled_mob = null
				M.update_canmove()
				M.lifeweb_locked = FALSE
				M.crucified = FALSE
				M.anchored = null
				M.remove_lifewebchain()
				M.update_icons()
				M.update_body()
				M.resting = 1
				return
		else 
			to_chat(user, "<span class='churchexcomm'>[pick(fnord)] I can't, I need the Vicar's Ring.</span>")
			return
