...
/mob/living/carbon/human/death(gibbed)
	...
	var/list/deathAlarmList = list("Baron", "Successor", "Heir", "Baroness", "Hand", "Vicar")
	
	if(src.job in deathAlarmList)
		var/obj/item/device/radio/intercom/a = new /obj/item/device/radio/intercom(null)// BS12 EDIT Arrivals Announcement Computer, rather than the AI.
		a.autosay("Alert: An important noble person has died.", "Enoch's Gate CTTU")
	...
	if(AffectedArea?.name == "Dungeon")
		...
		for(var/obj/item/device/radio/intercom/I in intercoms)
			playsound(I, 'sound/lfwbsounds/death_alarm.ogg', 50)
		a.autosay("Alert: A prisoner has died in the dungeon.", "Enoch's Gate CTTU")
	...
	if(src.job == "Vicar")
		for(var/mob/living/carbon/human/H in mob_list)
			if(H?.outsider == FALSE && H?.mind?.special_role != "tiamatrait" && !H?.siegesoldier)
				H?.add_event("deadbaron", /datum/happiness_event/misc/vicardead)
	if(april_fools)
	...
	if(src.isVampire || src.job == "Fortune Teller")
		var/atom/movable/overlay/animation = null
		monkeyizing = 1
		canmove = 0
		icon = null
